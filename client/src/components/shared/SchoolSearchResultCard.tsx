/**
 * School Search Result Card Component
 * Modern, refreshing design for comprehensive school student search results
 */
                ...
                <div className="pl-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {result.transport_assignment.slab_name && (
                      <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
                        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">Distance Slab</p>
                        <p className="text-base font-bold text-slate-900">{result.transport_assignment.slab_name}</p>
                      </div>
                    )}
                      ...
                      <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-6 border-2 border-purple-100">
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex-1">
                            ...
                            <div className="flex flex-wrap items-center gap-4 text-xs text-slate-600 ml-4">
                              <div>
                                <span className="text-slate-500">Actual:</span> <span className="font-bold text-slate-900">{formatCurrency(result.transport_fee_balance_summary.actual_fee)}</span>
                              </div>
                              {result.transport_fee_balance_summary.concession_amount > 0 && (
                                <>
                                  <span className="text-slate-300">•</span>
                                  <div>
                                    <span className="text-slate-500">Concession:</span> <span className="font-bold text-emerald-600">{formatCurrency(result.transport_fee_balance_summary.concession_amount)}</span>
                                  </div>
                                </>
                              )}
                              <span className="text-slate-300">•</span>
                              <div>
                                <span className="text-slate-500">Total:</span> <span className="font-bold text-slate-900">{formatCurrency(result.transport_fee_balance_summary.total_fee)}</span>
                              </div>
                            </div>
                          </div>
                          <div className="text-right ml-4">
                                ...
                                <div className="bg-white rounded-xl p-4 border-2 border-slate-200 shadow-sm">
                                  <div className="flex items-center justify-between mb-3">
                                    ...
                                    <Badge
                                      ...
                                      className={`text-xs ${result.transport_fee_balance_summary.term1.balance === 0 ? "bg-emerald-500" : ""}`}
                                    >
                                  ...
                                  <div className="space-y-2">
                                    <div className="flex justify-between text-xs">
                                      <span className="text-slate-500">Amount</span>
                                      <span className="font-bold text-slate-900">{formatCurrency(result.transport_fee_balance_summary.term1.amount)}</span>
                                    </div>
                                    <div className="flex justify-between text-xs">
                                      <span className="text-slate-500">Paid</span>
                                      <span className="font-bold text-emerald-600">{formatCurrency(result.transport_fee_balance_summary.term1.paid)}</span>
                                    </div>
                                    <div className="pt-2 border-t border-slate-200 flex justify-between">
                                      <span className="text-xs font-semibold text-slate-700">Balance</span>
                                      <span className={`text-sm font-bold ${result.transport_fee_balance_summary.term1.balance > 0 ? "text-red-600" : "text-emerald-600"}`}>
                                ...
                                <div className="bg-white rounded-xl p-4 border-2 border-slate-200 shadow-sm">
                                  <div className="flex items-center justify-between mb-3">
                                    ...
                                    <Badge
                                      ...
                                      className={`text-xs ${result.transport_fee_balance_summary.term2.balance === 0 ? "bg-emerald-500" : ""}`}
                                    >
                                  ...
                                  <div className="space-y-2">
                                    <div className="flex justify-between text-xs">
                                      <span className="text-slate-500">Amount</span>
                                      <span className="font-bold text-slate-900">{formatCurrency(result.transport_fee_balance_summary.term2.amount)}</span>
                                    </div>
                                    <div className="flex justify-between text-xs">
                                      <span className="text-slate-500">Paid</span>
                                      <span className="font-bold text-emerald-600">{formatCurrency(result.transport_fee_balance_summary.term2.paid)}</span>
                                    </div>
                                    <div className="pt-2 border-t border-slate-200 flex justify-between">
                                      <span className="text-xs font-semibold text-slate-700">Balance</span>
                                      <span className={`text-sm font-bold ${result.transport_fee_balance_summary.term2.balance > 0 ? "text-red-600" : "text-emerald-600"}`}>
