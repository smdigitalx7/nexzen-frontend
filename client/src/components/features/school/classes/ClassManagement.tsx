...
import { useSchoolClasses, useCreateSchoolClass, useUpdateSchoolClass } from '@/lib/hooks/school/use-school-classes';
...
const ClassesManagement = () => {
  const { data: backendClasses = [], isLoading: classesLoading } = useSchoolClasses();
  const { data: academicYears = [], isLoading: academicYearsLoading } = useAcademicYears();
  const [editingClass, setEditingClass] = useState<any>(null);
  const createClassMutation = useCreateSchoolClass();
  const updateClassMutation = useUpdateSchoolClass(editingClass?.class_id ?? 0);
  ...
  useEffect(() => {
    ...
    const mapped: UIClassRow[] = backendClasses.map((c: SchoolClassRead) => {
