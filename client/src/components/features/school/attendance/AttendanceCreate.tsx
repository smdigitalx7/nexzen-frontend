...
import { useBulkCreateSchoolAttendance, useBulkUpdateSchoolAttendance, useCreateSchoolAttendance } from '@/lib/hooks/school/use-school-attendance';
...
export default function AttendanceCreate() {
  ...
  const bulkUpdate = useBulkUpdateSchoolAttendance();
  const createSingle = useCreateSchoolAttendance();
    ...
    <Dialog open={bulkOpen} onOpenChange={setBulkOpen}>
      <DialogContent className="max-w-md">
        ...
        <DialogFooter>
          ...
          <Button disabled={!selectedClassId} onClick={async () => {
            if (!selectedClassId) { toast({ title: 'Error', description: 'Select class first', variant: 'destructive' }); return; }
            try {
              await bulkCreate.mutateAsync({
                class_id: selectedClassId,
                section_id: selectedSectionId ?? 0,
