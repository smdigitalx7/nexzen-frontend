...
import { useBusRouteNames } from "@/lib/hooks/useTransport";
...
export default function ReservationNew() {
  ...
  const handleSave = async (withPayment: boolean) => {
    ...
    try {
      const res: any = await SchoolService.createReservation(fd);
  ...
  const handleView = async (r: any) => {
    try {
      const data = await SchoolService.getReservation(Number(r.id));
  ...
  const handleEdit = async (r: any) => {
    try {
      const data: any = await SchoolService.getReservation(Number(r.id));
  ...
  const submitEdit = async () => {
    ...
    try {
      ...
      await SchoolService.updateReservation(Number(selectedReservation.id), fd);
  ...
  const handleConfirmCancel = async () => {
    ...
    try {
      if (selectedReservation?.id) {
        await SchoolService.updateReservationStatus(Number(selectedReservation.id), 'CANCELLED');
        // refresh list
        await loadReservations();
      }
    } catch (e: any) {
  ...
  const loadReservations = async () => {
    try {
      ...
      const res: any = await SchoolService.listReservations({ page: 1, page_size: 20 });
                        ...
                        <TableCell>
                          <Button
                            ...
                            onClick={async () => {
                              ...
                              try {
                                await SchoolService.updateReservationStatus(Number(r.id), to);
                                await loadReservations();
                              } catch (e: any) {
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={async () => {
                ...
                try {
                  await SchoolService.deleteReservation(Number(reservationToDelete.id));
                  await loadReservations();
                } catch (e: any) {
