...
export const SchoolStudentsService = {
  list(params?: SchoolStudentsListParams) {
    return Api.get<SchoolStudentsPaginatedResponse>(`/school/students`, params as Record<string, string | number | boolean | null | undefined> | undefined);
  },
  ...
  getByAdmission(admission_no: string) {
    return Api.get<SchoolStudentFullDetails>(`/school/students/admission-no/${admission_no}`);
  },
  ...
  update(student_id: number, payload: SchoolStudentUpdate) {
    return Api.put<SchoolStudentFullDetails>(`/school/students/${student_id}`, payload);
  },
