...
import type { CollegeClassCreate, CollegeClassList, CollegeClassResponse, CollegeClassUpdate, CollegeClassWithGroups } from "@/lib/types/college/index.ts";
...
export function useCreateCollegeClass() {
  ...
  return useMutation({
    mutationFn: (payload: CollegeClassCreate) => CollegeClassesService.create(payload) as Promise<CollegeClassResponse>,
    onSuccess: () => {
...
export function useUpdateCollegeClass(classId: number) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (payload: CollegeClassUpdate) => CollegeClassesService.update(classId, payload) as Promise<CollegeClassResponse>,
    onSuccess: () => {
...
export function useCollegeClassGroups(classId: number | null | undefined) {
  return useQuery({
    queryKey: typeof classId === "number" ? [...collegeKeys.classes.detail(classId), "groups"] : [...collegeKeys.classes.root(), "groups", "nil"],
    queryFn: () => CollegeClassesService.getGroups(classId as number) as Promise<CollegeClassWithGroups>,
...
export function useRemoveCollegeClassGroup(classId: number) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (groupId: number) => CollegeClassesService.removeGroup(classId, groupId),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: [...collegeKeys.classes.detail(classId), "groups"] });
    },
  });
}
