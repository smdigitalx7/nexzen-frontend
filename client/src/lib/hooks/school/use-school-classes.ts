...
export function useCreateSchoolClass() {
  ...
  return useMutation({
    mutationFn: (payload: SchoolClassCreate) => SchoolClassesService.create(payload) as Promise<SchoolClassRead>,
    onSuccess: () => {
...
export function useUpdateSchoolClass(classId: number) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (payload: SchoolClassUpdate) => SchoolClassesService.update(classId, payload) as Promise<SchoolClassRead>,
    onSuccess: () => {
...
export function useSchoolClassSubjects(classId: number | null | undefined) {
  return useQuery({
    queryKey: typeof classId === "number" ? [...schoolKeys.classes.detail(classId), "subjects"] : [...schoolKeys.classes.root(), "subjects", "nil"],
    queryFn: () => SchoolClassesService.getSubjects(classId as number) as Promise<SchoolClassWithSubjects>,
...
export function useDeleteSchoolClassSubject() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ classId, subjectId }: { classId: number; subjectId: number }) => SchoolClassesService.deleteClassSubject(classId, subjectId),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: schoolKeys.classes.root() });
    },
  });
