...
import { CollegeClassList, CollegeClassResponse, CollegeClassWithGroups, CollegeClassCreate, CollegeClassUpdate } from "@/features/college/types";
...
export const CollegeClassesService = {
  ...
  getGroups(class_id: number) {
    return Api.get<CollegeClassWithGroups>(`/college/classes/${class_id}/groups`);
  },

  // PUT /api/v1/college/classes/{class_id}
  update(class_id: number, payload: CollegeClassUpdate) {
    return Api.put<CollegeClassResponse>(`/college/classes/${class_id}`, payload);
  },

  // DELETE /api/v1/college/classes/{class_id}/groups/{group_id}
  removeGroup(class_id: number, group_id: number) {
    return Api.delete<void>(`/college/classes/${class_id}/groups/${group_id}`);
  },
};
