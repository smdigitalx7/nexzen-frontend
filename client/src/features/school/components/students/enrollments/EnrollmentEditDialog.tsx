...
import { useSchoolStudent, useUpdateSchoolStudent } from '@/features/school/hooks';
import type { SchoolStudentUpdate } from '@/features/school/types';
...
export const EnrollmentEditDialog = ({
  ...
  // Fetch student data
  const { data: studentData, isLoading: isLoadingStudent } = useSchoolStudent(studentId);
  
  // Update mutation
  const updateMutation = useUpdateSchoolStudent(studentId || 0);

  // Form state
  const [formData, setFormData] = useState<SchoolStudentUpdate>({
    student_name: '',
    aadhar_no: null,
    gender: null,
    dob: null,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updateMutation.mutateAsync(updatePayload as SchoolStudentUpdate);
      onOpenChange(false);
      if (onSuccess) {
        onSuccess();
      }
