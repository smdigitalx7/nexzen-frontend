...
import {
  ...
  useSchoolClassById,
  useDeleteSchoolClassSubject,
  useSchoolSubjects,
  useCreateSchoolClassSubject,
} from "@/features/school/hooks";
import { useToast } from "@/common/hooks/use-toast";
import type { ColumnDef } from "@tanstack/react-table";
import { createIconTextColumn } from "@/common/utils/factory/columnFactories";
...
export const ClassesTab = memo(
  ({
    ...
    const [selectedClass, setSelectedClass] = useState<SchoolClassRead | null>(
      null
    );
    ...
    const { data: classWithSubjects } = useSchoolClassById(
      isManageSubjectsOpen || isViewDialogOpen
        ? selectedClass?.class_id || null
        : null // ✅ Only fetch when dialog is open
    );
    ...
    const [optimisticSubjects, setOptimisticSubjects] = useState<
      { subject_id: number; subject_name: string }[] | null
    >(null);
    ...
    const validateClassForm = useCallback(
      (form: typeof initialClassForm) => {
    ...
    const handleManageSubjectsClick = useCallback(
      (classItem: SchoolClassRead) => {
      ...
      async (subjectId: number) => {
        ...
        setOptimisticSubjects((prev) => {
          ...
          return [
            ...prev,
            {
              subject_id: subjectToAdd.subject_id,
              subject_name: subjectToAdd.subject_name,
            },
          ];
        });
    ...
    const columns: ColumnDef<SchoolClassRead>[] = useMemo(
      () => [
        createIconTextColumn<SchoolClassRead>("class_name", {
          header: "Class Name",
          icon: BookOpen,
        }),
        {
    ...
    const canEditClass = useCanViewUIComponent(
      "classes",
      "button",
      "class-edit"
    );
          ...
          <DialogContent className="max-w-4xl">
            ...
            {selectedClass && (
              <div className="space-y-6">
                ...
                {/* Action Buttons */}
                <div className="flex justify-end gap-2 pt-4 border-t">
                  <Button variant="outline" onClick={closeManageSubjectsDialog}>
                    Close
                  </Button>
                </div>
              </div>
