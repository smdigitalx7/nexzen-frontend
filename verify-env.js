...
const checkEnvFile = () => {
  ...
  if (!existsSync(envFile)) {
    ...
    console.log('\n💡 Solution:');
    console.log('   1. Copy env.akshara.production to .env.production');
    console.log('   2. Run: cp env.akshara.production .env.production');
    process.exit(1);
  }
  ...
  // Check for Akshara-specific values
  const isAkshara = envContent.includes('Akshara');
  if (isAkshara) {
    console.log('\n✅ Akshara brand configuration detected');
    console.log('   Brand Name:', foundVars.find(v => v.name === 'VITE_BRAND_NAME')?.value);
  } else {
    console.log('\n⚠️  Warning: Akshara brand not detected in configuration');
  }
  ...
  console.log('\n' + '='.repeat(50));
  console.log('📊 Summary:');
  console.log(`   Environment Variables: ${foundVars.length}/${requiredVars.length} ✅`);
  console.log(`   Assets: ${assetsFound}/${requiredAssets.length} ${assetsFound === requiredAssets.length ? '✅' : '⚠️'}`);
  
  if (foundVars.length === requiredVars.length && assetsFound === requiredAssets.length) {
